...
class SinusIterableWithGenerator:
    ...
    def __iter__(self):
        # локальная переменная внутри метода (не класса) для хранения состояния
        x = 0.0
        # в данном случае - бесконечный цикл, но мог бы быть с условием окончания последовательности
        while True:
            x += math.pi * 2 / self.steps
            length = self.width / 2 + self.width / 2 * math.sin(x) + 1
            # вместо return используется yield для создания генератора - объекта, который
            # сохраняет переменные (х) и логику (цикл while) текущего метода (__iter__);
            # логика выполняется при вызове метода __next__ у созданного генератора;
            # преимущества:
...
if __name__ == '__main__':
    ...
    main_iter = SinusIterableWithGenerator(12, 18)
